...
export function getImageAspectRatio(colSpan: number, rowSpan: number): string {
  ...
  var minDim = Math.min(cardW, cardH);
  var sw = Math.round(minDim * CARD_STROKE_RATIO);
  if (sw < 1) sw = 1;
  var inset = Math.round(sw * 1.6);
  var textH = cardH - Math.round(cardH * CARD_IMAGE_RATIO);
  var imgW = cardW - 2 * inset;
  var imgH = cardH - textH - 2 * inset;
